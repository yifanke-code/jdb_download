...
RE_PAGE_LINKS    = re.compile(
    r'<a\s{1}href="([^"]+(?="class="box")).+?((?<=<strong>)[^<]+)</stron.+?'
...
def convert_linux_to_windows_path(linux_path: str) -> str | None:
    if not linux_path:
        return None
    for prefix, win in PATH_MAP.items():
        if linux_path.startswith(prefix):
            return linux_path.replace(prefix, win).replace('//', '/')
    return linux_path
...
class LocalScanWorker:
    ...
    def process_one(self, cb) -> bool:
        ...
        try:
            ...
            items   = parsed['items']
            if not items:
                self._advance(cb, f'[LOCAL] Cannot parse album: {aid}')
                return True
            ...
            cb('log', f'[LOCAL] Added: {real_aid} - {title} ({len(parsed["actors"])} actors)')
...
class JdbDownloader(QMainWindow):
    ...
    def _init_ui(self):
        ...
        for w in (self.btn_start, self.btn_pause, self.btn_continue, self.btn_skip,
                  self.btn_download_mag, self.btn_save_album, self.btn_stop):
            cl.addWidget(w)
        main.addLayout(cl)
    ...
    def on_table_double_click(self, row: int, col: int):
        ...
        pyperclip.copy(mag)
        self.log(f'[COPY] Magnet copied: {mag[:50]}...')
        if self.combo_dest.currentText() == 'aria2' and self.aria2:
            try:
    ...
    def go_to_album(self):
        ...
        album = AlbumInfo.from_parsed(link, aid, parsed, title)
        if aid.upper() not in self.id_list:
        ...
        self.current_album.magnet_links = parse_magnet_items(source)
        self.paused_album = self.current_album
    ...
    def check_collection_and_open_browser(self, aid: str):
        if self.df_collection.empty or 'ID' not in self.df_collection.columns:
            self.log('[COLLECTION] df_collection empty or missing ID')
            return
        row = self.df_collection[self.df_collection['ID'].str.upper() == aid.upper()]
        if row.empty:
            self.log(f'[COLLECTION] {aid} not found')
            return
        path_val = row['path'].iloc[0] if 'path' in row.columns else None
        if not path_val:
            self.log(f'[COLLECTION] {aid} has no path')
            return
        win_path = convert_linux_to_windows_path(path_val)
        self.log(f'[COLLECTION] Win path: {win_path}')
        try:
            if os.path.exists(win_path):
                subprocess.Popen(f'explorer /select,"{win_path}"')
                self.log(f'[COLLECTION] Opened: {win_path}')
            else:
                folder = os.path.dirname(win_path)
                if os.path.exists(folder):
                    subprocess.Popen(f'explorer "{folder}"')
                    self.log(f'[COLLECTION] Opened folder: {folder}')
                else:
                    self.log(f'[COLLECTION] Path not found: {folder}')
        except Exception as e:
    ...
    def check_page_change(self):
        # Skip during local scan
        if self._is_local_scanning:
            return
        if not self.browser:
            return
        try:
            ...
            self.current_album = AlbumInfo.from_parsed(url_str, aid, parsed, title)
            self.current_album.magnet_links = mag_links_raw
            self.paused_album = self.current_album
    ...
    def add_album_to_memory(self, album: AlbumInfo):
        ...
        for alink, actor in actors:
            av = 'Collection' if len(actors) > 1 else (str(actor).strip() if actor else '')
            row = {
                'AV':            av.strip(),
                ...
                'path':          0,
                'ID':            aid.upper().strip(),
                'Cast':          actor[1].strip() if actor else '',
            ...
            self.df_javdb = pd.concat([self.df_javdb, pd.DataFrame([row])], ignore_index=True)

        self.id_list.add(aid.upper())  # keep set in sync
        ...
        def worker_callback(event, data):
            try:
                if event == 'log':
                    ...
                    QTimer.singleShot(10, self._local_scan_next)
                elif event == 'finished':
                    self._is_local_scanning = False
    ...
    def on_local_finished(self, albums: list[AlbumInfo]):
        ...
        if not albums:
            ...
            return
        self.log(f'[LOCAL] Total: {len(albums)} albums — saving...')
        for album in albums:
            self.add_album_to_memory(album)
        self.save_javdb_to_file()
        self.log(f'[LOCAL] DONE! {len(albums)} albums saved')
